...
var directory = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "ForceBreak-SmokeTest");
...
try
{
    ...
    installed = true;
    Check(Directory.GetFiles(directory).Select(Path.GetFileName).SequenceEqual(new[] { "ForceBreak.exe" }), "installed application is also a single EXE");
    ...
    using (var registration = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\ForceBreak"))
    ...
    Check(duplicateRejected && File.Exists(Path.Combine(directory, "ForceBreak.exe")), "existing installation is not overwritten");
    ...
    Check(!File.Exists(Path.Combine(NativeInstaller.Shortcuts, "Force Break - Recovery.lnk")), "no graphical emergency shortcut");
    using (var recover = Process.Start(new ProcessStartInfo(Path.Combine(directory, "ForceBreak.exe"))
