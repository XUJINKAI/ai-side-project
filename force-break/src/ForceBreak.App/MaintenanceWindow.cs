...
internal static class MaintenanceLauncher
{
    ...
    public static async Task Run(MaintenanceAction action, string directory, string? emergencyConfirmation = null)
    {
        ...
        try
        {
            ...
            if (File.Exists(Path.Combine(AppContext.BaseDirectory, "ForceBreak.dll")))
                throw new InvalidOperationException("安装管理请使用发布的单文件 ForceBreak.exe。");
            temporary = Path.Combine(Path.GetTempPath(), "ForceBreak-" + Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(temporary);
            var helper = Path.Combine(temporary, "ForceBreak.exe");
    ...
    public static void CleanupTemporaryHelper()
    {
        ...
        if (Path.GetFileName(directory).StartsWith("ForceBreak-", StringComparison.Ordinal)
        ...
        Loaded += async (_, _) =>
        {
            try
            {
                ...
                if (overwrite && MessageBox.Show(this,
                    "发现已有安装或数据，是否覆盖安装？\n\n将更新程序并修复安装登记，保留现有配置、进度、承诺和暂停状态。",
