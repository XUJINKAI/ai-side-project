...
public sealed record Schedule
{
    ...
    public void Validate()
    {
        ...
        if (Commitment >= Reminder) throw new ArgumentException("承诺时间必须早于首次提醒。");
        if (Reminder >= Bedtime) throw new ArgumentException("首次提醒必须早于限制开始时间。");
        if (Release >= Commitment)
            throw new ArgumentException("次日解除时间必须早于承诺时间，避免两晚重叠。");
...
public sealed class PlannerState
{
    ...
    public void Validate()
    {
        if (Version != 1 || Break is null) throw new InvalidDataException("Invalid planner state.");
        Schedule.Validate();
        if (!double.IsFinite(Break.WorkSeconds) || Break.WorkSeconds < 0) throw new InvalidDataException("Invalid break timer.");
    }
}

// Exact frozen commitment timestamps never leave the service.
