...
{
    public ReminderWindow(DateTimeOffset bedtime, bool preview, System.Drawing.Rectangle? screen = null, bool isBreak = false)
    {
        Title = preview ? "睡前提醒预览" : isBreak ? "该休息了" : "该睡觉了";
        ...
        content.Children.Add(new TextBlock { Text = preview ? "睡前提醒 · 预览" : isBreak ? "离开屏幕，休息一下。" : "把今天留在今天。", FontSize = 30, Margin = new Thickness(0, 0, 0, 20) });
        ...
        void Update()
        {
            ...
            countdown.Text = remaining > TimeSpan.Zero ? $"{(int)remaining.TotalMinutes:00}:{remaining.Seconds:00}" : isBreak ? "休息时间" : "晚安";
